...
def load_model(model_name: str,
    ...
    if not isinstance(models[model_name]['gdrive_id'], list):
        model = models[model_name]['model']()
        if (dataset_ == BenchmarkDataset.imagenet or dataset_ == BenchmarkDataset.imagenet3d) and 'Standard' in model_name:
            return model.eval()
...
def _safe_load_state_dict(model: nn.Module, model_name: str,
    ...
    try:
        ...
        if (model_name in known_failing_models or dataset_ == BenchmarkDataset.imagenet or dataset_ == BenchmarkDataset.imagenet3d
...
def clean_metrics(model: nn.Module,
                   x: torch.Tensor,
                   y: torch.Tensor,
                   batch_size: int = 100,
                   num_classes=10,
    ...
    with torch.no_grad():
        for counter in range(n_batches):
            ...
            # print("y_curr.type():",y_curr.type())
            # print("y_curr:",y_curr)
            y_hot = nn.functional.one_hot(y_curr, num_classes=num_classes)
            # print("y_hot.shape:",y_hot.shape)
            # assert False
            acc += (output.max(1)[1] == y_curr).float().sum()
            bri += brier(output, y_hot)
            nll += nll_f(output, y_hot)
    acc_avg = acc.item() / x.shape[0]
    bri_avg = bri.item() / x.shape[0]
    nll_avg = nll.item() / x.shape[0]

    return acc_avg, bri_avg, nll_avg
